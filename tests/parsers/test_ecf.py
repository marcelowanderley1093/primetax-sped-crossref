"""Testes do parser ECF — foco em Bloco 9 (Bug-001)."""
...
class TestParserEcf:
    def test_bloco9_divergencia_detectada(
        self, fixture_ecf_bloco9_divergencia, tmp_path,
    ):
        """Fixture declara 9900.0000=99 mas real=1 → divergência detectada."""
        res = _importar(fixture_ecf_bloco9_divergencia, tmp_path)
    ...
    def test_contagens_reais_cumulativo(
        ...
        """contagens_reais cumulativo por reg_tipo."""
        res = _importar(fixture_ecf_bloco9_divergencia, tmp_path)
        assert res.contagens_reais.get("0000") == 1
        assert res.contagens_reais.get("0010") == 1
